...
try:
    ...
    try:
        ...
        print(
            "Error: Neither AppIndicator3 nor AyatanaAppIndicator3 namespace available."
        )
    ...
    def parse_usb_block(self, lines):
        ...
        for line in lines:
            if line.startswith("T:"):
                if m := re.search(r"Spd=\s*(\S+)", line):
                    entry["speed"] = m.group(1)
                if m := re.search(r"Bus=(\d+)", line):
                    entry["bus_info"] = f"Bus {m.group(1)}"
            elif line.startswith("D:"):
                if m := re.search(r"Ver=\s*(\d+\.\d+)", line):
                ...
                if "Manufacturer=" in line:
                    entry["manufacturer"] = line.split("Manufacturer=")[1].strip()
                elif "Product=" in line:
                    entry["product"] = line.split("Product=")[1].strip()
                elif "SerialNumber=" in line:
                    entry["serial"] = line.split("SerialNumber=")[1].strip()
        ...
        return (
            entry
            if (
                entry.get("vidpid") or entry.get("product") or entry.get("manufacturer")
            )
            else None
        )
...
class UsbMenuApp:
    def __init__(self):
        ...
        self.indicator = AppIndicator3.Indicator.new(
            self.app_id, self.icon, AppIndicator3.IndicatorCategory.APPLICATION_STATUS
        )
...
def main():
    ...
    print("USB Connection Information started. Check the system tray for the icon.")
    UsbMenuApp()
    Gtk.main()
